use crate::parse::input::InputId;
use analysis_utils::{Arena, impl_id_type};
use grammar_utils::{AstNode, SyntaxNode, TreeArc};
...
impl<N: AstNode> AstId<N> {
    ...
    pub(crate) fn to_node(&self, db: &dyn ParseDatabase) -> TreeArc<N> {
        let syntax_node = db.ast_id_to_node(self.input_id, self.input_ast_id.raw);
...
pub struct ErasedInputAstId(u32);
impl_arena_id!(ErasedInputAstId);

/// Maps items' `SyntaxNode`s to `ErasedInputAstId`s and back.
#[derive(Debug, PartialEq, Eq)]
pub struct AstIdMap {
    arena: Arena<ErasedInputAstId, SyntaxNodePtr>,
}
...
impl AstIdMap {
    ...
    fn alloc(&mut self, item: &SyntaxNode) -> ErasedInputAstId {
